...
# FSL3 event data (event IDs: 218=START, 227=DATA, 237=STOP, 487=JOIN)
FSL3_START_EVENT_1 = b'\x00\xda"5|\x1d\x00\x0bq\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
FSL3_DATA_EVENT_1 = b'\x00\xe3"<\x8a\xfa\x00\x0b\xf1\xf3\x00\x0e\x00\x00\t\xc4\x1dl\x0eO\x00\x00x\x8f\rh'
FSL3_STOP_EVENT_1 = b'\x00\xed"<\x8a\xec\x00\x0b\xf1\xf0@gP\x1d>1\x97H5\x00\x00\x00\x00\x00\x00\x00'
FSL3_JOIN_EVENT_1 = b'\x01\xe7"5\x83A\x00\x0bq\x82\x1e\x02\x04\x01APR0AL2X\x00\x00\x00U'

FSL3_STOP_EVENT_2 = b'\x00\xed";\xf0<\x00\x0b\xe6\xc0A9JV?dfd@\x89\x7f\x12\x00\x00\x00\x00'
FSL3_STOP_EVENT_3 = b'\x00\xed";\xe5\xb0\x00\x0b\xe5\xd7A.Q&?dfd@\x89\x7f\x12\x00\x00\x00\x00'


class TestProcessCGMReadingFSL3(unittest.TestCase):
    """Test FSL3 CGM event parsing and processing"""
    maxDiff = None
    ...
    def test_fsl3_start_event_parses(self):
        """Test that FSL3 START event parses correctly"""
        events = [Event(FSL3_START_EVENT_1)]

        self.assertEqual(type(events[0]), eventtypes.LidCgmStartSessionFsl3)
        self.assertEqual(events[0].sessionstartflag, 0)
        self.assertEqual(events[0].seqNum, 749825)

    def test_fsl3_data_event_parses(self):
        """Test that FSL3 DATA event parses with correct field values"""
        events = [Event(FSL3_DATA_EVENT_1)]

        self.assertEqual(type(events[0]), eventtypes.LidCgmDataFsl3)
        # Verify raw fields are extracted
        self.assertEqual(events[0].glucosevaluestatusRaw, 0)
        self.assertEqual(events[0].cgmdatatypeRaw, 0)
        self.assertEqual(events[0].algorithmstateRaw, 108)
        self.assertEqual(events[0].currentglucosedisplayvalue, 79)
        self.assertEqual(events[0].RSSI, 29)
        self.assertEqual(events[0].egvtimestamp, 574393082)
        self.assertEqual(events[0].interval, 143)
        self.assertEqual(events[0].seqNum, 782835)

    def test_fsl3_data_enum_transforms(self):
        """Test that FSL3 DATA enums transform correctly"""
        events = [Event(FSL3_DATA_EVENT_1)]

        # Test glucose value status enum (valid)
        self.assertEqual(events[0].glucosevaluestatus, eventtypes.LidCgmDataFsl3.GlucosevaluestatusEnum.PreciseValue)
        # Test algorithm state enum (108 is not a valid enum value, so returns None)
        self.assertIsNone(events[0].algorithmstate)

    def test_fsl3_data_bitmask_transforms(self):
        """Test that FSL3 DATA bitmask transforms correctly"""
        events = [Event(FSL3_DATA_EVENT_1)]

        # cgmDataTypeRaw = 14 = bits 1 and 2 set
        # bit 1 = Backfill, bit 2 (or 3?) should also be set for 14
        # 14 in binary is 0b1110 = bits 1, 2, 3 set
        # From mapping: bit 1=Backfill, but 14 doesn't directly map
        # The event has cgmDataTypeRaw=14, check what that resolves to
        cgm_type = events[0].cgmdatatype
        self.assertIsNotNone(cgm_type)

    def test_fsl3_data_rate_transform(self):
        """Test that FSL3 DATA rate field applies ratio transform"""
        events = [Event(FSL3_DATA_EVENT_1)]

        # rateRaw is at offset 0, should be int16
        # Expected: raw * 0.1
        self.assertEqual(events[0].rateRaw, 14)  # int16 from bytes 0-1
        self.assertAlmostEqual(events[0].rate, 1.4)

    def test_fsl3_data_to_nightscout_entry(self):
        """Test that FSL3 DATA event converts to Nightscout entry"""
        events = [Event(FSL3_DATA_EVENT_1)]
        ...
        self.assertEqual(len(p), 1)
        self.assertIn('sgv', p[0])
        self.assertEqual(p[0]['sgv'], 79)

    def test_fsl3_stop_event_parses(self):
        """Test that FSL3 STOP event parses correctly"""
        events = [Event(FSL3_STOP_EVENT_1)]

        self.assertEqual(type(events[0]), eventtypes.LidCgmStopSessionFsl3)
        self.assertEqual(events[0].sessionstopreason, 64)
        self.assertEqual(events[0].seqNum, 782832)
    ...
    def test_fsl3_join_event_parses(self):
        ...
        self.assertEqual(events[0].joinreason, 30)
        self.assertEqual(events[0].sensoridentifier, 65)
        self.assertEqual(events[0].seqNum, 749954)

    def test_multiple_fsl3_events(self):
        """Test processing multiple FSL3 events together"""
        events = [
            Event(FSL3_START_EVENT_1),
            Event(FSL3_DATA_EVENT_1),
            Event(FSL3_STOP_EVENT_1)
        ]

        # Verify correct types
        self.assertEqual(type(events[0]), eventtypes.LidCgmStartSessionFsl3)
        self.assertEqual(type(events[1]), eventtypes.LidCgmDataFsl3)
        self.assertEqual(type(events[2]), eventtypes.LidCgmStopSessionFsl3)

    def test_fsl3_offset_negative_handling(self):
        """Test that negative offset (-2) correctly reads from header region"""
        events = [Event(FSL3_DATA_EVENT_1)]

        # egvTimestamp uses offset -2 (absolute header position 2)
        # This should read from bytes 2-5 of the 26-byte event
        self.assertIsInstance(events[0].egvtimestamp, int)
        # Verify it's a reasonable timestamp value (not garbage)
        self.assertGreater(events[0].egvtimestamp, 0)
